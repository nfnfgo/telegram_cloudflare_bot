...
if __name__=='__main__':
    ...
    # rewrite part
    rv_list=['CF_API_KEY'] # define what keywords should be rewrite
    ...
    with open(r_path.r_path+'/config/cf_config_example.py','r') as f:
        text=f.read()
    for vari,value in res_dict.items():
        text=text.replace(vari,value)
    with open(r_path.r_path+'/config/cf_config.py','w') as f:
        f.write(text)
